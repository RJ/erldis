...
enter_loop(Mod, Options, State, ServerName, Timeout, Backoff) ->
    ...
    Queue = erldis_priority_queue:new(),
...
process_next_msg(GS2State = #gs2_state { time          = Time,
                                         ...
                                         queue         = Queue }) ->
    case erldis_priority_queue:out(Queue) of
        {{value, Msg}, Queue1} ->
...
in({'$gen_cast', Msg}, GS2State = #gs2_state { prioritise_cast = PC,
                                               queue           = Queue }) ->
    GS2State #gs2_state { queue = erldis_priority_queue:in(
                                    {'$gen_cast', Msg},
...
format_status(Opt, StatusData) ->
    ...
    [{header, Header},
     {data, [{"Status", SysState},
             {"Parent", Parent},
             {"Logged events", Log},
             {"Queued messages", erldis_priority_queue:to_list(Queue)}]} |
     Specfic].
